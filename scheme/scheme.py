...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        operands = rest.map(lambda func: scheme_eval(func, env))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('uh oh spaghettios!')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        body = expressions.second.first
        env.define(target, scheme_eval(body, env))
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        function, parameter, body = target.first, target.second, expressions.second
        env.define(function, LambdaProcedure(parameter, body, env))
        return function
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions:
            current_expr = scheme_eval(expressions.first, env)
            if current_expr is False:
                return current_expr
            else:
                expressions = expressions.second
        return current_expr
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second == nil:
                return test
            while clause.second:
                value = scheme_eval(clause.second.first, env)
...
def make_let_frame(bindings, env):
    ...
    for binding in bindings:
        value = scheme_eval(binding.second, env)
