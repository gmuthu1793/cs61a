...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        if not message.startswith(magic_word):
            ...
            alist = [x for x in args]
            if len(alist) == 1:
                return 'Thanks for asking, but I know not how to {0}.'.format(message)
            if alist[0].startswith(magic_word):
                if alist[0][7:] == 'vend':
                    return self.obj.obj.vend()
                if alist[0][7:] == 'deposit':
                    return self.obj.obj.deposit(alist[1])
